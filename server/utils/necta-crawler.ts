...
/**
 * Extract school entries from a NECTA index page.
 * Returns each school's center number, name (from link text), and its individual page URL.
 */
function extractSchoolsFromIndex(html: string, baseUrl: string): Array<{
  centerNumber: string;
  schoolName: string;
  url: string;
}> {
  ...
  const result: Array<{ centerNumber: string; schoolName: string; url: string }> = [];
  ...
  $('a[href]').each((_, el) => {
    ...
    const centerNumber = centerMatch[1].toUpperCase();
    if (seen.has(centerNumber)) return;
    seen.add(centerNumber);

    const absolute  = new URL(href, baseUrl).href;
    // School name: prefer link text, fall back to surrounding td text, then center number
    const linkText  = $(el).text().trim();
    const parentTd  = $(el).closest('td');
    const tdText    = parentTd.text().trim();
    const schoolName = linkText || tdText || centerNumber;

    result.push({ centerNumber, schoolName, url: absolute });
  });
...
function parseDivRow(cells: cheerio.Cheerio<any>, $: cheerio.CheerioAPI, offset = 1): DivCount {
  ...
  return { div1, div2, div3, div4, div0, total: div1 + div2 + div3 + div4 + div0 };
}
...
function parseSchoolPage(html: string, centerNumber: string): School {
  ...
  const rawTitle   = $('title').text() || $('h1,h2,h3,h4').first().text() || centerNumber;
  const school_name = rawTitle.replace(/results|enquir[iy]es?|necta|\d{4}/gi, '').trim() || centerNumber;

  // Summary table — first table: M/F/T rows
  ...
  if (totalRow.total === 0 && (male.total > 0 || female.total > 0)) {
    totalRow = {
      ...
      div0: male.div0 + female.div0, total: male.total + female.total,
    };
  }
  ...
  // Student results table — second table (or first if no summary table)
  const students: Student[] = [];
  const resultTable = $('table').length > 1 ? $('table').eq(1) : $('table').first();
  const headerRow   = resultTable.find('tr').first();
  const headers     = headerRow.find('th, td')
    .map((_, el) => $(el).text().trim().toUpperCase()).get();
  ...
  resultTable.find('tr').slice(1).each((_, row) => {
    ...
    cells.each((i, cell) => { if (i >= subjStart) subjectParts.push($(cell).text().trim()); });
    const subjects = subjectParts.filter(Boolean).join(' ').trim();

    students.push({ index_number: cno, name: cno, sex, aggregate, division, subjects });
  });
...
/**
 * Exported: fetch and parse an individual school page on demand.
 * Used by the result-schools route for lazy student data loading.
 */
export async function fetchAndParseSchool(
  year: number,
  examType: string,
  centerNumber: string,
): Promise<School> {
  // Reconstruct the individual school URL from year/examType/centerNumber
  const url = `${NECTA_ORIGIN}/results/${year}/${examType}/results/${centerNumber}.htm`;
  const html = await fetchHtml(url);
  return parseSchoolPage(html, centerNumber);
}
  ...
  <div class="results-index" id="results-index">
    <div class="table-responsive">
      <table class="results-table results-index-table">
        <thead>
          <tr>
            <th>No.</th><th>Centre No.</th><th>School Name</th>
            <th>Div I</th><th>Div II</th><th>Div III</th><th>Div IV</th><th>Abs/Fail</th>
          </tr>
        </thead>
...
/**
 * Phase 1 crawl: parse the NECTA index page only — no individual school HTTP requests.
 * All schools are saved instantly with placeholder data; student data is fetched
 * on demand the first time a user visits a school's detail page.
 */
export async function crawlNectaResults(
  ...
  const schoolList = extractSchoolsFromIndex(indexHtml, url);
  const total      = schoolList.length;

  if (total === 0) throw new Error('No school links found on this page.');

  onEvent({ stage: 'crawling', progress: 0, total, message: `Found ${total} schools. Saving to database...` });

  // Save all schools from index in parallel batches (no individual page fetches needed)
  const BATCH = 50;
  const schools: School[] = [];
  let processed = 0;

  for (let i = 0; i < schoolList.length; i += BATCH) {
    const batch = schoolList.slice(i, i + BATCH);

    await Promise.all(batch.map(async (entry) => {
      // Placeholder school record — students will be fetched on demand
      const school: School = {
        center_number: entry.centerNumber,
        school_name:   entry.schoolName,
        summary: { div1: 0, div2: 0, div3: 0, div4: 0, div0: 0 },
        students: [],
      };
      schools.push(school);

      if (onSchoolCrawled) {
        await onSchoolCrawled(school, processed + 1, total);
      }
      ...
      onEvent({
        ...
        progress: processed,
        total,
        message: `Saving schools... ${processed}/${total}`,
      });
    }));
  }
  ...
  const structured_data: StructuredData = {
    ...
    total_students: 0, // student data loaded on demand
    schools,
  };
  ...
  let meta_description = `Angalia matokeo ya ${examLabel} mwaka ${year} Tanzania. Tafuta kwa shule au index number. ${total} shule.`;
...
- meta_description: Swahili+English SEO description (120-160 chars) mentioning year, exam, ${total} schools
...
JSON only, no explanation.`
      ...
      // AI failed — keep defaults
    }
  }

  onEvent({ stage: 'building', progress: 1, total: 1, message: 'Building content...' });
  ...
  return {
    title,
    slug: slugify(`${exam_type}-${year}-results`),
    year,
    exam_type,
    source_url: url,
    content,
